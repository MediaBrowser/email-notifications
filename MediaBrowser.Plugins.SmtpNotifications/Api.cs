...
{
    [Route("/EmailNotifications", "POST", Summary = "Saves a notification", IsHidden = true)]
    [Authenticated(Roles = "Admin")]
    public class SaveNotification : EmailNotificationInfo, IReturnVoid
    {
    }

    [Route("/EmailNotifications/Default", "GET", Summary = "Gets default notification info", IsHidden = true)]
    [Authenticated(Roles = "Admin")]
    public class GetDefaultNotificationInfo : IReturn<EmailNotificationInfo>
    {
    }

    [Route("/EmailNotifications/Send/Test", "POST", Summary = "Sends a notification request, for testing only", IsHidden = true)]
    [Authenticated(Roles = "Admin")]
    public class SendNotificationTest : EmailNotificationInfo
    {
    }

    [Route("/EmailNotifications", "DELETE", Summary = "Deletes a notification", IsHidden = true)]
    [Authenticated(Roles = "Admin")]
    ...
    public class NotificationsApi : IService, IRequiresRequest
    {
        ...
        public void Post(SaveNotification request)
        {
            ...
            var updatedNotification = _json.DeserializeFromString<EmailNotificationInfo>(_json.SerializeToString(request));
        ...
        private InternalNotificationRequest GetSampleNotificationRequest(EmailNotificationInfo request)
        {
        ...
        {
            return new Notifier(_config, _logger).SendNotification(GetSampleNotificationRequest(request), Request.CancellationToken);
        }
    }
}
