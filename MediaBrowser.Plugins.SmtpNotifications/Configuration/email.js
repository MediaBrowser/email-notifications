define(['baseView', 'dom', 'globalize', 'require', 'emby-linkbutton', 'emby-button', 'listViewStyle', 'paper-icon-button-light'], function (BaseView, dom, globalize, require) {
    ...
    function getEntryEditorJsPath() {
        return Dashboard.getConfigurationResourceUrl('emailnotificationeditorjs');
    }

    function getDefaultEntry() {
        return ApiClient.getJSON(ApiClient.getUrl('EmailNotifications/Default'));
    }
    ...
    function getConfiguredNotifications() {
        return ApiClient.getNamedConfiguration('emailnotifications').then(function (config) {
    ...
    function deleteEntry(view, button) {
        ...
        return require(['confirm']).then(function (responses) {
            ...
            return confirm({
                ...
                ApiClient.ajax({
                    url: ApiClient.getUrl('EmailNotifications', { Id: id }),
                    type: 'DELETE'
                }).then(function () {
    ...
    function getEntryHtml(entry, allEventTypes) {
        ...
        html += '<h3 class="listItemBodyText">' + entry.EmailTo + '</h3>';
