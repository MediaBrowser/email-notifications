define(['dom', 'dialogHelper', 'loading', 'apphost', 'layoutManager', 'connectionManager', 'appRouter', 'globalize', 'emby-checkbox', 'emby-input', 'emby-select', 'paper-icon-button-light', 'emby-select', 'material-icons', 'formDialogStyle', 'emby-button', 'emby-linkbutton', 'flexStyles', 'emby-scroller'], function (dom, dialogHelper, loading, appHost, layoutManager, connectionManager, appRouter, globalize) {
    ...
    function onSubmit(e) {
        ...
        apiClient.ajax({
            ...
            url: ApiClient.getUrl('EmailNotifications'),
            type: 'POST',
    ...
    function sendEntryTest() {
        ...
        apiClient.ajax({
            ...
            url: ApiClient.getUrl('EmailNotifications/Send/Test'),
            type: 'POST',
    ...
    function getEntry(instance) {
        ...
        obj.EmailFrom = dlg.querySelector('.txtEmailFrom').value;
        obj.EmailTo = dlg.querySelector('.txtEmailTo').value;
        obj.Server = dlg.querySelector('.txtServer').value;
        obj.Port = dlg.querySelector('.txtPort').value || null;
        obj.EnableSSL = dlg.querySelector('.chkEnableSSL').checked;
        obj.Username = dlg.querySelector('.txtUsername').value;
        obj.Password = dlg.querySelector('.txtPassword').value;
    ...
    function fillData(entry, dlg, apiClient) {
        ...
        dlg.querySelector('.txtEmailFrom').value = entry.EmailFrom || '';
        dlg.querySelector('.txtEmailTo').value = entry.EmailTo || '';
        dlg.querySelector('.txtServer').value = entry.Server || '';
        dlg.querySelector('.txtPort').value = entry.Port || '';
        dlg.querySelector('.chkEnableSSL').checked = entry.EnableSSL;
        dlg.querySelector('.txtUsername').value = entry.Username || '';
        dlg.querySelector('.txtPassword').value = entry.Password || '';
    ...
    EntryEditor.prototype.show = function (options) {
        ...
        return require(['text!' + Dashboard.getConfigurationResourceUrl('emaileditortemplate')]).then(function (responses) {
